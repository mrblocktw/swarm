...
func TestMultiChequeSimulation(t *testing.T) {
	...
	cashChequeDone := make(chan *CashoutRequest, 1)
	defer close(cashChequeDone)
	creditorSvc.swap.cashoutProcessor.setCashoutDoneChan(cashChequeDone)
	...
	for i := 0; i < msgAmount; i++ {
		...
		if balanceAfterMessage <= -paymentThreshold {
			...
			if err := waitForChequeProcessed(t, params.backend, counter, lastCount, debitorSvc.swap.peers[creditor], expectedPayout, cashChequeDone); err != nil {
				t.Fatal(err)
			}
...
func waitForChequeProcessed(t *testing.T, backend *swapTestBackend, counter metrics.Counter, lastCount int64, p *Peer, expectedLastPayout uint64, cashChequeDone <-chan *CashoutRequest) error {
	t.Helper()
